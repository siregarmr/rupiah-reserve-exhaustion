...
# using semicolon-separated HS codes. Aggregates monthly totals and writes
# year-to-date monthly averages to bps_trade_latest.csv.
...
# Generate all two‑digit HS codes from 01 to 99, semicolon-separated
...
def extract_month_number(bulan_str: str) -> int:
    """Extract month number from string like '[02] Februari' or plain name."""
...
def fetch_monthly_totals(api_key: str, trade_type: str, year: int):
    """
    Fetch monthly totals for all HS codes using a single API call.
    trade_type: 'export' (sumber=1) or 'import' (sumber=2)
    Returns dict {month_number: total_value}
    """
    sumber = "1" if trade_type == "export" else "2"
    url = (
        f"{BPS_BASE_URL}/sumber/{sumber}/kodehs/{HS_CODES_ALL}/jenishs/1"
    ...
    if data.get("status") != "OK":
        print(f"API status not OK for {trade_type}: {data.get('status')}", file=sys.stderr)
        return {}
...
def main():
    ...
    print(f"Fetching monthly trade data for {year} (single request per trade type, all HS codes)...")
    exports = fetch_monthly_totals(BPS_API_KEY, "export", year)
    imports = fetch_monthly_totals(BPS_API_KEY, "import", year)

    if not exports or not imports:
        print("ERROR: Failed to retrieve export or import data.", file=sys.stderr)
        sys.exit(1)

    # Use months that exist in both datasets and are ≤ current month
